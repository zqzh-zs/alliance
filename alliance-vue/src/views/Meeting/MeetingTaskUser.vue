<template>
  <div>
    ...
    <el-dialog v-model="editDialogVisible" title="编辑会议">
      <el-form :model="editForm" label-width="100px">
        ...
        <el-form-item label="开始时间">
          <el-date-picker v-model="editForm.start_time" type="datetime" />
        </el-form-item>
...
const userInfo = JSON.parse(localStorage.getItem('alliance-user') || '{}')
...
const fetchMeetings = async () => {
  ...
  try {
    ...
    if (res.code === 200) {
      tableData.value = res.data.list // Updated field
      total.value = res.data.total // Updated field
    } else {
...
const openEditDialog = async (row) => {
  try {
    ...
    console.log(res)
    if (res.code === 200) {
      // Adjusted to access res.data.data
...
const handleImageUpload = async (file) => {
  ...
  formData.append('cover', file)  // 上传接口字段名是 cover
  ...
  try {
    ...
    console.log(res)  // 调试用，查看返回结构

    if (res.code === 200 && res.data.code === 200) {
      editForm.cover = res.data.data.url  // 正确获取图片路径
  ...
  try {
    await request({
      ...
      data: {
        ...
        imageUrl: editForm.cover,  // 更新接口字段名是 imageUrl
        type: editForm.type
      }
    })
